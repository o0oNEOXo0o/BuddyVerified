...
function buddyverified_inner_meta_box() {
	...
	<table id="bp-verified" cellspacing="3px" style="border-collapse: collapse;">
		...
		<tbody>
			...
			<?php do_action( 'buddyverified_meta_box_fields', 'user', $user_id, $meta ); ?>
		</tbody>
	</table>
...
function buddyverified_save_metabox( $array ) {
	...
	if ( isset( $_POST['save'] ) && 'Update Profile' === $_POST['save'] ) {
		...
		$bp_verified_arr = apply_filters( 'bp_user_verified_arr', $bp_verified_arr, $_POST );
...
function buddyverified_admin_css() {
	...
	<style>
		...
		table#bp-verified td {
			padding: 5px;
		}

		table#bp-verified td.pad {
			padding: 10px;
		}
	</style>
	<?php
